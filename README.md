...
The export panel pops up from saved `TBCGearExporterDB` data. It shows saved bag/bank counts, scan/debug controls, source buttons for **All**, **Bags**, **Bank**, and **Gear**, format buttons for **AI**, **JSON**, **Markdown**, and **Text**, filter buttons for **All Q**, **Rare+**, **Epic**, and **Gear Epic**, and tabs for **Overview**, visual items, stats analysis, and copyable text export. The default Overview page gives a compact character, inventory, talent, stat, quality, category, and role snapshot so the popup is useful before opening raw export text. The item browser uses saved item icons, colored names, counts, item levels, locations, quality, type, and item-link tooltips so you can review gear without staring at raw JSON. The Stats Analysis page shows the current character sheet snapshot, hit/crit/defense/avoidance/power models, top strategy-book role lenses, benchmark statuses, race and raid/party notes, and gear stat highlights directly inside the addon, using localized display labels instead of internal model/status tokens. Markdown export is now a readable report with a quick summary, role snapshot, collapsible character details, and Wowhead-linked item tables; AI Text and JSON keep the full structured dataset for external tools. GUI labels, status text, tooltips, slash help, and scan chat lines follow the WoW client locale for English, simplified Chinese, and traditional Chinese clients.
