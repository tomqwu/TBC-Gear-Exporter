...
![Coverage](https://img.shields.io/badge/coverage-99.17%25-brightgreen)
...
The export panel pops up from saved `TBCGearExporterDB` data. It shows saved bag/bank counts, scan/debug controls, source buttons for **All**, **Bags**, **Bank**, and **Gear**, format buttons for **AI**, **JSON**, **Markdown**, and **Text**, filter buttons for **All Q**, **Rare+**, **Epic**, and **Gear Epic**, and tabs for **Overview**, **Gear Advice**, visual items, stats analysis, **P2 Guide**, and copyable text export. The Gear Advice page offers role and mitigation/threat, throughput/longevity, or cap/output views, plus real item icons, tooltips, quality-colored names, visible-stat gains/losses, benchmark impact, model kind, and item-data completeness. Current models rank candidates but do not issue definitive upgrade verdicts. The P2 Guide displays route evidence separately from the score model so a WoWSims reference set is never presented as proof that a candidate swap was simulated. Spell damage and spell power are compared as one offensive stat. Level-70 combat ratings are converted into the unit used by each tracked benchmark. Item level and quality do not add score. Set bonuses, sockets, enchants, proc effects, rotations, and encounters remain outside the current evaluator. Markdown and plain-text exports follow the client locale; AI Text and JSON retain the full structured dataset and model contract for external tools.

## Phase 2 strategy database

Version 0.4.8 introduces database version 6 and the first strict engine contract. All 28 role records now declare their actual score model. Balance, Retribution, and Arcane use field-verified P2 static EP tables from the pinned WoWSims revision; Hunter is correctly labeled as a shared P1 BM/SV estimate, including the unsupported reuse for Marksmanship; the other 22 roles are labeled ordered-stat heuristics. No current role may emit a definitive upgrade verdict. Talent representation coverage is now separate from calculation-rule coverage, route evidence is separate from scoring provenance, item-data completeness is no longer called model evidence, and item level/quality no longer add arbitrary score. Earlier cap, weapon-layout, and Feral Bear fixes remain in place.
...
This is currently a transparent candidate-ranking and export engine, not a full combat simulator or global loadout optimizer. It does not model encounter timelines, rotations, hidden proc rates, set bonuses, enchants, gems, temporary buffs, or every talent's exact combat formula. Those remain explicit manual/AI review inputs instead of being silently invented.
...
- Stable tags named `v*`, for example `v0.4.8`, build a GitHub Release using notes from `CHANGELOG.md`, then automatically publish the BCC package to CurseForge as a stable release. Routine `main` pushes do not create development tags or uploads.
