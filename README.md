...
The export panel pops up from saved `TBCGearExporterDB` data. It shows saved bag/bank counts, scan/debug controls, source buttons for **All**, **Bags**, **Bank**, and **Gear**, format buttons for **AI**, **JSON**, **Markdown**, and **Text**, filter buttons for **All Q**, **Rare+**, **Epic**, and **Gear Epic**, and tabs for **Overview**, **Gear Advice**, visual items, stats analysis, **P2 Guide**, and copyable text export. The Gear Advice page offers role and strategy views, real item icons, tooltips, quality-colored names, an auditable `stat delta x weight = contribution` explanation, explicitly unscored changes, and an **Other Candidate Decisions** section that explains every remaining gear item with its formula and rejection or ranking reason. It also shows contextual effect choices, active Tier 5 sets, set-threshold warnings, guide-route gaps, talent-adjusted benchmark math, model provenance, and item-data completeness. Current models rank candidates but do not issue definitive upgrade verdicts. Every role now includes a **PvP / Arena** view. A loadout with at least 80 resilience across at least three equipped pieces selects it automatically; the engine then uses same-level hit references, values resilience/stamina and caster spell penetration, labels resilience losses as tradeoffs, and suppresses PvE simulator targets. Holy Paladin adds Mixed Healing, Flash of Light, and Holy Light views. Tank modes keep balanced, mitigation, and threat decisions separate, including contextual trinket effects. Mitigation and threat now declare independent objective-stat sets: specialized lists rank only their selected dimension, while cross-dimension gains remain visible in the candidate audit instead of masquerading as upgrades. Survival Hunter recognizes Surefooted, Lightning Reflexes, and Expose Weakness; the report keeps paper-doll hit, talent hit, effective hit, and the 9% target distinct, while Expose Weakness raid AP remains contextual rather than being invented as personal EP. The P2 Guide displays route evidence separately from the score model so a reference set is never presented as proof that a candidate swap was simulated. Spell damage and spell power are compared as one offensive stat. Level-70 combat ratings are converted into the unit used by each tracked benchmark. A hard gate such as crit immunity binds in every strategy mode: a swap that would push a currently met gate back below target is rejected into the candidate audit rather than ranked, no matter how well it scores on the selected objective. Every recommended candidate is marked with where you already own it, such as bank or bags with the exact bag and slot, so a recommendation is never mistaken for an item you still have to farm. Item level and quality do not add score. Empty sockets carry a fixed nominal placeholder weight so a socketed item is not treated as statless, while the gems themselves, enchants, proc effects, rotations, and encounters remain outside the evaluator. Markdown and plain-text exports follow the client locale; AI Text and JSON retain the full structured dataset and model contract for external tools.

## Phase 2 strategy database

Version 0.5.9 uses database version 11, strategy-book schema version 8, and recommendation engine version 21. It gives all three tank roles explicit survival and threat objectives, ranks specialized views by the selected objective rather than a blended total, and exports both the objective and off-objective score. Version 0.5.8 added the shared, source-labeled PvP context layer for all 28 roles.

Version 0.5.5 introduced the underlying database version 9 and recommendation engine version 18. Every recommendation carries a reconciled score breakdown with signed stat deltas, active role/mode weights, per-stat contributions, net visible-stat score, and excluded changes with an explicit reason. Stat maluses subtract from scores instead of being silently dropped. Strategy-mode multipliers scale only weights the selected role declares — generic hit/crit/haste multipliers fall through to school-specific weights — so tank threat mode cannot invent spell-stat weights and Maximum output mode cannot lower a school-specific haste valuation. Class weapon and relic proficiency gates reject candidates the class cannot wield before ranking, and block-model tank roles refuse to trade the shield away: a non-shield off-hand candidate gets the explicit `shield_required` rejection instead of posing as a tradeoff. Generic hit/crit/haste ratings are treated as the physical-only stats they are in TBC — they never inherit spell-school weights and no longer count toward the spell-hit benchmark. Benchmark-gap and talent boosts, like mode multipliers, scale only weights the role declares. Paper-doll expertise points convert to percent before the 6.5% benchmark check, `equipped_gear` JSON scores use the same role slot-gating as every other exported score, and stat weights keep four decimals so every printed `delta x weight = contribution` line reproduces its result. Empty sockets carry a fixed nominal placeholder weight (4 per socket) so socketed items are not treated as statless; the gems themselves remain out of scope. Ring and trinket candidates are compared against both equipped slots. Curated effects carry a decision dimension and localized scenario: equal mode affinity across different uses becomes a yellow `effect_context` tradeoff instead of a green visible-stat upgrade, while mitigation and threat modes rank by the full affinity delta. Every other evaluated gear item has a candidate-audit status plus the available formula and cap projection for benchmark gates, low score, same-slot ranking, role/class mismatch, loadout conflicts, weaker effects, contextual tradeoffs, unscorable data, or unsupported slots. Benchmarks preserve `raw_observed`, `talent_bonus`, and `effective_observed` separately and use each role's declared target. Contextual talent conversions, such as Expose Weakness attack power per physical attacker while active, are exported but explicitly excluded from EP. The contextual layer contains 14 source-linked item effects and all 17 Tier 5 class sets, covering every one of the 28 supported PvE roles. Set ownership is gated by both class and role, so shared role keys such as Restoration Druid and Restoration Shaman cannot leak across classes. The GUI and exports show currently equipped set counts and their 2/4-piece activation states. A 5-to-4 swap correctly keeps the 4-piece bonus, while a 4-to-3 swap is marked as a tradeoff. The database still tracks 32 reference sets and 526 target slots, with route gaps exported independently from bag-item recommendations.
...
- Stable tags named `v*`, for example `v0.5.9`, build a GitHub Release using notes from `CHANGELOG.md`, then automatically publish the BCC package to CurseForge as a stable release. Routine `main` pushes do not create development tags or uploads.
