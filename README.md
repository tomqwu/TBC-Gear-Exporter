...
Version 0.5.6 adds localized icon-led Stats Analysis panels and recommendation engine version 19. Positive visible-stat scores below 8 are labeled as small improvements, while larger heuristic results remain estimates. Tank defense rating now projects into both combined critical-hit reduction and the visible dodge/parry/block subtotal, retaining partial avoidance value after crit immunity without applying the contextual table boost twice.
...
- Stable tags named `v*`, for example `v0.5.6`, build a GitHub Release using notes from `CHANGELOG.md`, then automatically publish the BCC package to CurseForge as a stable release. Routine `main` pushes do not create development tags or uploads.
