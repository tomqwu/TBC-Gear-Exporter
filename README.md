...
Current talents are saved into `current_talents` with tree point summary, primary tree, total/unspent points, and selected talent ranks. Live character-sheet stats are saved into `character_stats` with race, faction, level, raid/party context, attributes, armor, defense, attack power, combat ratings, crit/avoidance chances, spell damage, healing, and mana regen. Talent and character stat snapshots refresh when bags or bank are scanned, when exports are generated, and when WoW fires a talent update event.
