...
local function Mode(key, labels, multipliers, focus, objective)
    return { key = key, labels = labels, multipliers = multipliers or {}, focus = focus or {}, objective = objective }
end
...
local TANK_SURVIVAL_STATS = {
    S.stamina, S.armor, S.bonusArmor, S.defense, S.dodge, S.parry,
    S.block, S.blockValue, S.resilience, S.agility,
}

local function TankModes(threat, threatStats)
    ...
    return WithPvPMode({
        ...
        Mode("mitigation", Labels("Mitigation / progression", "减伤 / 开荒", "減傷 / 開荒"), {
            ...
            key = "survival",
            labels = Labels("Survival / mitigation", "生存 / 减伤", "生存 / 減傷"),
            statTokens = TANK_SURVIVAL_STATS,
        }),
        Mode("threat", Labels("Threat / farm", "仇恨 / Farm", "仇恨 / Farm"), threatMultipliers, { "required_caps", "threat", "tempo" }, {
            key = "threat",
            labels = Labels("Threat output", "仇恨输出", "仇恨輸出"),
            statTokens = threatStats or {},
        }),
    })
end
